...
import { LogOut, User, Star, Users, UserCircle, BarChart3, Wand2, Trophy, MessageCircle, SkipBack, SkipForward, Globe, Sparkles, Palette, Film, Radio, Bell, GitBranch, Heart, Brain, Disc3, Shield, Upload } from 'lucide-react';
...
const UploadPanel = lazy(() => import('./components/UploadPanel').then(m => ({ default: m.UploadPanel })));
const FigmaAIDesignGuide = lazy(() => import('./components/FigmaAIDesignGuide').then(m => ({ default: m.FigmaAIDesignGuide })));
...
export function AppInner() {
  ...
  type PanelType =
    ...
    | 'smart-playlist' | 'live-session' | 'showcase' | 'album-store' | 'e2ee-setup' | 'secondary-market' | 'upload' | 'design-guide';
  ...
  const showUpload = activePanel === 'upload';
  const showDesignGuide = activePanel === 'design-guide';
  ...
  return (
    ...
    >
      ...
      {showUpload && (
        <UploadPanel
          ...
          onUploadComplete={(media) => {
            console.log('[Upload] Media uploaded:', media.id, media.title);
            if (media.signedUrl && media.mediaType === 'audio') {
              const newTrack: Track = {
                id: media.id,
                title: media.title,
                artist: media.artist,
                ...
                albumArt: media.coverSignedUrl || playlist[0]?.albumArt || '',
                audioUrl: media.signedUrl,
                lyrics: [
                  { time: 0, text: '\u266B \u266B \u266B', translation: media.title, emotion: 'happy' as Emotion },
                  { time: 30, text: media.artist, translation: media.album, emotion: 'calm' as Emotion },
                ],
                chordSet: Math.floor(Math.random() * 4),
                color: `hsl(${Math.random() * 360}, 70%, 50%)`,
              };
              ...
              shouldAutoPlayRef.current = true;
              setCurrentTrackIndex(playlist.length);
              // Async: fetch existing lyrics for this uploaded track (may exist from previous edits)
              apiFetch<{ lyrics: Array<{ time: number; text: string; translation?: string; emotion?: string }> }>(
                `/upload/media/${media.id}/lyrics`
              ).then(res => {
                if (res?.lyrics?.length > 0) {
                  const ll: LyricLine[] = res.lyrics.map(l => ({
                    time: l.time, text: l.text,
                    translation: l.translation || '',
                    emotion: (l.emotion || 'neutral') as Emotion,
                  }));
                  setPlaylist(prev => prev.map(t => t.id === media.id ? { ...t, lyrics: ll } : t));
                }
              }).catch(() => {}); // Non-blocking; placeholder lyrics remain
            }
          }}
          onLyricsUpdate={(mediaId, lyrics) => {
            // Sync saved lyrics from UploadPanel editor back into the playlist track
            console.log('[Upload] Lyrics synced to player for', mediaId, ':', lyrics.length, 'lines');
            const ll: LyricLine[] = lyrics.map(l => ({
              time: l.time, text: l.text,
              translation: l.translation || '',
              emotion: (l.emotion || 'neutral') as Emotion,
            }));
            setPlaylist(prev => prev.map(t => t.id === mediaId ? { ...t, lyrics: ll } : t));
          }}
        />
      )}
      </Suspense>
        ...
        >
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden lg:flex items-center gap-1">
              ...
              <button
                onClick={() => { openPanel('upload'); haptics.trigger('light'); }}
                ...
                title={lang === 'zh' ? '上传音乐/视频' : 'Upload Music/Video'}
                aria-label="Upload"
              >
      ...
      <MobileDiscoverHub
        ...
        onOpenDesignGuide={() => openPanel('design-guide')}
      />
      )}
      </Suspense>
